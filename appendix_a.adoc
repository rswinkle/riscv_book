...
It's nowhere near as nice as RARS and is poorly documented and maintained.

UC Berkeley is the designer of RISC-V and so was one of the first colleges
to start teaching RISC-V, before RARS existed. Now that they've built up
a curriculum around Venus and invested in their own fork they don't want
to switch.
...
The only documentation I can find is again from ThaumicMekanism's wiki
...
RARS.{venus_ecalls}  In RARS, you use `a7` to select the ecall leaving the lower a
...
on passing arguments in CS 61C's documentation is inaccurate.  Just
